...
function OffensivePlays() {
  ...
  const loadPlays = async () => {
    // TODO: 从后端API加载战术数据
    // 目前使用本地存储模拟
    const stored = localStorage.getItem(`offensive_plays_${teamId}`);
    if (stored) {
  ...
  const savePlays = async (updatedPlays: Play[]) => {
    // TODO: 保存到后端API
    // 目前使用本地存储模拟
    localStorage.setItem(`offensive_plays_${teamId}`, JSON.stringify(updatedPlays));
        ...
        <div className="container mx-auto px-4 py-4">
          <div className="flex items-center justify-between">
            <div className="flex items-center space-x-4">
              ...
              <span className="text-white">进攻战术 - {team?.name || '加载中...'}</span>
            </div>
            {canEdit && (
              <button
                onClick={() => {
      ...
      <div className="container mx-auto px-4 py-6">
        {plays.length === 0 ? (
          <div className="text-center py-16 bg-white rounded-lg shadow">
            ...
            <p className="text-xl text-gray-500 mb-4">暂无进攻战术</p>
            {canEdit && (
              <button
