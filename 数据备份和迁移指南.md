...
### 当前数据存储
- **数据库文件**：`backend/database/basketball.db`
- **数据库类型**：SQLite（单文件数据库）
- **包含数据**：所有球队、球员、比赛记录、统计数据

### 重要说明
⚠️ **SQLite 数据库文件默认在 `.gitignore` 中，不会被上传到 Git！**

这意味着：
- ✅ 代码会上传到 Git
- ❌ **数据库文件不会上传到 Git**
- ⚠️ 部署到新服务器时，数据库是空的，需要重新创建或迁移数据
...
### 方案二：使用 Git LFS（大文件存储）

如果数据库文件较大，可以使用 Git LFS：
...
### Q: 为什么上传到 Git 后数据没了？
A: 因为数据库文件在 `.gitignore` 中，不会被 Git 跟踪。这是正常的，因为：
- 数据库文件通常很大
- 包含敏感数据
- 不应该在版本控制中
